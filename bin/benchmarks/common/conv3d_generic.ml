module Make(M : Owl_types.Ndarray_Basic) = struct
  ...
  let _ = Printf.printf "Evaluating conv3d operation\n"
  ...
  let _ =
    ...
    then failwith "ERROR! Must provide the stride, and dimensions of the input and kernel as follows:\n
          [batch; input_column; input_row; input_depth; input_channel; kernel_column; kernel_row; kernel_depth; output_channel; stride]"
  ...
  let dims_kernel = Array.map int_of_string
      ...
      [|Sys.argv.(6); Sys.argv.(7); Sys.argv.(8); Sys.argv.(5); Sys.argv.(9)|]
  let stride = int_of_string Sys.argv.(10)
  ...
  let output_shape = (M.conv3d input kernel stride) |> M.shape
  let output' = M.ones output_shape

  let wrap_fun () =
    begin
      for rep = 0 to 10 do
        let _ = M.conv3d input kernel stride in
        let _ = M.conv3d_backward_input input kernel stride output' in
        let _ = M.conv3d_backward_kernel input kernel stride output' in
        ()
      done
    end
