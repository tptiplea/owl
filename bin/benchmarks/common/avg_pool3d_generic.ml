module Make(M : Owl_types.Ndarray_Basic) = struct
  ...
  let _ = Printf.printf "Evaluating avg_pool3d operation\n"
  ...
  let kernel = Array.map int_of_string [|Sys.argv.(6); Sys.argv.(7);|]
  ...
  let output_shape = (M.avg_pool3d input kernel stride) |> M.shape
  let output' = M.ones output_shape

  let wrap_fun () =
    begin
      for rep = 0 to 10 do
        let _ = M.avg_pool3d input kernel stride in
        let _ = M.avg_pool3d_backward Owl_types.SAME input kernel stride output' in
        ()
      done
    end
